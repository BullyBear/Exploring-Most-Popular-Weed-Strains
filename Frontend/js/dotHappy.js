...
// create a tooltip
var tooltip = d3.select("#dotHappy")
  .append("div")
    .style("opacity", 0)
    .attr("class", "tooltipHappyDot")
...
var data = d3.csv('../static/data/leafly/leafly/Frontend/assets/data/happy_dot_modified.csv', function(data) {
   ...
   groups
    .on("mousemove", function(d) {
      var htmlText = "<span style='font-weight:bold; font-size:14px;'>" + d.name + "</span>" + "<br/>" +
        ...
        // .style("left", htmlText + "px")     
        // .style("top", htmlText + "px");
      // .style("left", d3.select(this).attr("cx") + document.getElementsByClassName("groups").offsetLeft + "px")     
      // .style("top", d3.select(this).attr("cy") + document.getElementsByClassName("groups").offsetTop + "px");
      // .style("left", (parseInt(d3.select(this).attr("cx")) + document.getElementById("dotHappy").offsetLeft) + "px")     
      // .style("top", (parseInt(d3.select(this).attr("cy")) + document.getElementById("dotHappy").offsetTop) + "px");                 
    })                  
...
ScrollTrigger.addEventListener("refreshInit", () => gsap.set(".dot", {y: 0}));    // DO I WANT THIS ? 
...
const killStress = (direction) => {

    if (direction === "up") {

      console.log("killStress")

      d3.selectAll(".chartStress").style("opacity", 0);

      d3.selectAll("#divImgurlTwo").style("opacity", 0);

      d3.selectAll("#divImgurlFive").style("opacity", 0);
