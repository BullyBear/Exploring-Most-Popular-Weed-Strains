...
// create a tooltip
var tooltip = d3.select("#dotEyes")
  .append("div")
    .style("opacity", 0)
    .attr("class", "tooltipEyesDot")
...
var data = d3.csv('../static/data/leafly/leafly/Frontend/assets/data/eyes_dot_modified.csv', function(data) {
...
var svg = d3.select("#dotEyes")
    .append("svg")
    .attr("class", "eyesDotSvg")
...
// x.domain([28, 100]);
x.domain(data.map(d => d.Eyes));
y.domain([5, 33]);
// y.domain(data.map(d => d.thc));

var groups = svg.selectAll(".groupsThree")
    .data(data)
    .enter()
    .append("g")
    .attr("class", "groupsThree")
    .attr("nameofrow", function(d){
        return d.name;
    })
    .attr("transform", function(d) {
        // return "translate(" + x(d.day) + ".0)";
        return "translate(" + x(d.Eyes) + ".0)";
    });
...
// gsap.set(".dotEyes", {y: -100});
gsap.set(".dotEyes", {y: 500});



ScrollTrigger.batch(".dotEyes", {
...
ScrollTrigger.addEventListener("refreshInit", () => gsap.set(".dotEyes", {y: 0}));
