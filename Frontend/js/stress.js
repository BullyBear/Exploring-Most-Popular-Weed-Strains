...
// create a tooltip
var tooltip = d3.select(".chartStress")
  .append("div")
    .style("opacity", 0)
...
var mouseover = function(d) {
  ...
  htmlText = "<span style='font-weight:bold; font-size:14px;'>" + d.name + "</span>" + "<br/>" +
              ...
              if (d.imgurl) {
                ...
                d3.select("#divImgurlFive").transition().duration(200).attr("src", d.imgurl)
              }

              else {
                d3.select("#divImgurlFive").transition().duration(200).attr("src", '/Backend/python_scripts/assets/data/anxious.png')
              }

              if (d.img_url_two) {
                d3.select("#divImgurlTwo").transition().duration(200).attr("src", d.img_url_two)
              }
              
              else {
                d3.select("#divImgurlTwo").transition().duration(200).attr("src", '/Backend/python_scripts/assets/data/potleaf.jpg')
              }
...
const data = d3.csv('../static/data/leafly/leafly/Backend/python_scripts/assets/data/modified_data_stress.csv', function(data){
  ...
  const boxes = uncount(data, d => d.Stress_Modified);
  ...
  // scaleColor = d3.scaleOrdinal()
    ...
    // const graph = d3.select(".chartStress");

    const graph = d3.select(".chartStress")
                      .style("opacity", 1);


    d3.select("#divImgurlTwo")
                      .style("opacity", 1);

    d3.select("#divImgurlFive")
                      .style("opacity", 1);



    const group = graph
      .selectAll(".containerThree")
      .data(nest)
...
let anim = gsap.to(".boxTwo", 1, {
  scrollTrigger: ".boxTwo", // start the animation when ".box" enters the viewport (once)
  scale: 1,
  ease: Back.easeOut,
  stagger: {
    grid: "auto",
...
const resetVizStress = (direction) => {

    if (direction === "up") {

      console.log("resetVizStress")
